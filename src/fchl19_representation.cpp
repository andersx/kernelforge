...
size_t idx2(size_t i, size_t j, size_t ncols) {
    return i * ncols + j;
}
...
void decay_matrix(const std::vector<double> &rmat, double invrc, size_t natoms,
...
//  - eta2, eta3, zeta, rcut, acut, two_body_decay, three_body_decay, three_body_weight: as in
//  Fortran
// Output:
        ...
        for (long long ii = 0; ii < static_cast<long long>(natoms); ++ii) {
            ...
            for (size_t j = i + 1; j < natoms; ++j) {
                ...
                const double inv_pref =
                    decay_ij / (sigma * SQRT_2PI * std::pow(rij, two_body_decay));
    ...
    for (size_t i = 0; i < natoms; ++i) {
        ...
        for (size_t j = i + 1; j < natoms; ++j) {
            ...
            for (int t = 0; t < 3; ++t) {
                ...
                const double ddecay = dx * 0.5 * PI *
                                      std::sin(PI * rij * (rcut > 0 ? 1.0 / rcut : 0.0)) *
                ...
                for (size_t k = 0; k < nbasis2; ++k) {
                    ...
                    const double A =
                        (term1 + term2) * (exp_ln[k] / (Rs2[k] * sigma * std::sqrt(PI) * 2.0)) -
                        (exp_ln[k] * eta2 * dx) / (Rs2[k] * (s2 * std::sqrt(PI)) * sigma *
...
void flocal_kernel(const std::vector<double> &x1,  // (nm1, max_atoms1, rep_size)
...
void flocal_kernel_symmetric(const std::vector<double> &x,  // (nm, max_atoms, rep_size)
                             const std::vector<int> &q,     // (nm, max_atoms)
                             const std::vector<int> &n,     // (nm)
                             int nm, int max_atoms, int rep_size, double sigma,
