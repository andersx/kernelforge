...
py::array_t<double> generate_fchl_acsf_py(const py::array &coords,     // (n,3) or (3n,)
    ...
    const size_t rep_size =
        fchl19::compute_rep_size(elements.size(), Rs2_v.size(), Rs3_v.size(), Ts_v.size());
    ...
    std::vector<double> rep;
    {
        ...
        fchl19::generate_fchl_acsf(coords_v, z_v, elements, Rs2_v, Rs3_v, Ts_v, eta2, eta3, zeta,
...
static void flatten_inputs(const py::list &X_list, const py::list &Q_list, std::vector<double> &X,
                           std::vector<int> &Q, std::vector<std::size_t> &offsets,
                           std::size_t &d_out) {
    ...
    for (std::size_t m = 0; m < nm; ++m) {
        ...
        if (x.ndim() != 2)
            throw std::runtime_error("Each rep must be 2D: (n_atoms × d)");
        if (q.ndim() != 1)
            throw std::runtime_error("Each charges array must be 1D");
        ...
        if (!d_set) {
            ...
            throw std::runtime_error("All molecules must share the same rep dimension d");
        if (static_cast<std::size_t>(q.shape(0)) != n)
            throw std::runtime_error("charges length mismatch");
        total += n;
        offsets[m + 1] = total;
    }
    ...
    for (std::size_t m = 0; m < nm; ++m) {
        ...
        for (std::size_t i = 0; i < n; ++i)
            Q[cursor + i] = static_cast<int>(pq[i]);
        cursor += n;
    }
}
...
static py::tuple generate_fchl_acsf_rep_and_grad_py(const py::array &coords,     // (n,3)
    ...
    {
        py::gil_scoped_release release;
        fchl19::generate_fchl_acsf_and_gradients(coords_v, z_v, elements, Rs2_v, Rs3_v, Ts_v, eta2,
...
static py::array_t<double> flocal_kernel_py(
    ...
    fchl19::flocal_kernel(x1v, x2v, q1v, q2v, n1v, n2v, nm1, nm2, max_atoms1, max_atoms2, rep_size,
                          sigma, Kptr);
...
static py::array_t<double> fatomic_local_gradient_kernel_py(
    ...
    {
        py::gil_scoped_release release;
        fchl19::fatomic_local_gradient_kernel(x1v, x2v, dX2v, q1v, q2v, n1v, n2v, nm1, nm2,
...
static py::array_t<double> fgdml_kernel_py(
    ...
    {
        py::gil_scoped_release release;
        fchl19::fgdml_kernel(x1v, x2v, dx1v, dx2v, q1v, q2v, n1v, n2v, nm1, nm2, max_atoms1,
...
static py::array_t<double> fgdml_kernel_symm_py(
    ...
    {
        py::gil_scoped_release release;
        fchl19::fgdml_kernel_symmetric_lower(x1v, dx1v, q1v, n1v, nm1, max_atoms1, rep, naq1, sigma,
                                             Kptr);
    }
...
static py::array_t<double> flocal_kernel_symm_rfp_py(
    ...
    fchl19::flocal_kernel_symmetric_rfp(xv, qv, nv, nm, max_atoms, rep_size, sigma, arf_ptr);
...
PYBIND11_MODULE(_fchl19, m) {
    ...
    m.def("fgdml_kernel_symm", &fgdml_kernel_symm_py, py::arg("x1"), py::arg("dx1"), py::arg("q1"),
...
Returns:
  K: (naq2, naq1) row-major, where
    ...
    m.def("fgdml_kernel", &fgdml_kernel_py, py::arg("x1"), py::arg("x2"), py::arg("dx1"),
...
Returns:
  K: (naq2, naq1) row-major, where
    ...
    m.def("fatomic_local_gradient_kernel", &fatomic_local_gradient_kernel_py, py::arg("x1"),
...
Returns:
  K: (nm1, naq2) where naq2 = 3 * sum(n2)
    ...
    m.def("compute_rep_size", &fchl19::compute_rep_size, py::arg("nelements"), py::arg("nbasis2"),
...
(rep, grad)
  ...
  grad: (n_atoms, rep_size, n_atoms, 3)
    ...
    m.def("flocal_kernel", &flocal_kernel_py, py::arg("x1"), py::arg("x2"), py::arg("q1"),
          py::arg("q2"), py::arg("n1"), py::arg("n2"), py::arg("sigma"));
    m.def("flocal_kernel_symm", &flocal_kernel_symm_py, py::arg("x1"), py::arg("q1"), py::arg("n1"),
          py::arg("sigma"));
    m.def("flocal_kernel_symm_rfp", &flocal_kernel_symm_rfp_py, py::arg("x1"), py::arg("q1"),
          py::arg("n1"), py::arg("sigma"),
