// cuda_fchl19_repr.cu — GPU-accelerated FCHL19 forward representation (FP32).
//
// Public function (namespace kf::fchl19):
//
//   generate_fchl_acsf_cuda(coords, Q, N, nelements, nRs2, nRs3, nFourier, ...)
...
//   Block = BLOCK_SZ (128) threads.
//   Two-body : threads stripe over neighbors j in [0, natoms).
//   Three-body: thread 0 builds (j,k) triplet list in shared memory;
//               all threads then stripe over triplets.
//
// Shared memory layout per block (dynamic):
//   sh_coords  [max_atoms * 3]        float32
//   sh_Z       [max_atoms]            int32
//   sh_D       [max_atoms * max_atoms] float32   (full pairwise distance matrix)
//   sh_rep     [rep_size]             float32   (accumulator, init 0, atomicAdd)
//   sh_tripj   [max_pairs]            int16
//   sh_tripk   [max_pairs]            int16
//   sh_ntrip   [1]                    int32     (4-byte aligned)
//
// For QM7B defaults (max_atoms=23, rep_size=984, max_pairs=253):
//   total shared memory ≈ 7.4 KB — well within the 48 KB limit.
...
// ---------------------------------------------------------------------------
// Device helper: symmetric element-pair index
...
__global__ static void fchl19_forward_kernel(
    ...
    int max_pairs                           // max_atoms * (max_atoms-1) / 2
) {
    ...
    // ALL threads in the block share the same (m, i), so this is not divergent.
    // -----------------------------------------------------------------------
    if (i >= natoms) {
    ...
    // -----------------------------------------------------------------------
    // Dynamic shared memory layout (byte offsets, all 4-byte aligned)
    // -----------------------------------------------------------------------
    extern __shared__ char smem[];

    const int off_coords = 0;
    const int off_Z      = off_coords + max_atoms * 3 * (int)sizeof(float);
    const int off_D      = off_Z      + max_atoms     * (int)sizeof(int);
    const int off_rep    = off_D      + max_atoms * max_atoms * (int)sizeof(float);
    const int off_tripj  = off_rep    + rep_size  * (int)sizeof(float);
    const int off_tripk  = off_tripj  + max_pairs * (int)sizeof(short);
    // align ntrip to 4-byte boundary
    const int off_ntrip  = (off_tripk + max_pairs * (int)sizeof(short) + 3) & ~3;

    float  *sh_coords = (float *)(smem + off_coords);
    int    *sh_Z      = (int   *)(smem + off_Z);
    float  *sh_D      = (float *)(smem + off_D);
    float  *sh_rep    = (float *)(smem + off_rep);
    short  *sh_tripj  = (short *)(smem + off_tripj);
    short  *sh_tripk  = (short *)(smem + off_tripk);
    int    *sh_ntrip  = (int   *)(smem + off_ntrip);

    // -----------------------------------------------------------------------
    // Load coordinates and element indices for this molecule
    // -----------------------------------------------------------------------
    ...
    // Initialise accumulator and triplet counter
    for (int r = tid; r < rep_size; r += BLOCK_SZ)
        sh_rep[r] = 0.0f;
    if (tid == 0) *sh_ntrip = 0;
    ...
    // -----------------------------------------------------------------------
    // Compute full pairwise distance matrix (stored with max_atoms stride)
    // -----------------------------------------------------------------------
    const int n2 = natoms * natoms;
    for (int p = tid; p < n2; p += BLOCK_SZ) {
        const int ai = p / natoms;
        const int aj = p % natoms;
        if (ai == aj) {
            sh_D[ai * max_atoms + aj] = 0.0f;
        } else {
            const float dx = sh_coords[aj * 3 + 0] - sh_coords[ai * 3 + 0];
            const float dy = sh_coords[aj * 3 + 1] - sh_coords[ai * 3 + 1];
            const float dz = sh_coords[aj * 3 + 2] - sh_coords[ai * 3 + 2];
            sh_D[ai * max_atoms + aj] = sqrtf(dx * dx + dy * dy + dz * dz);
        }
    }
    ...
    // -----------------------------------------------------------------------
    // Build triplet list (thread 0) — concurrent with two-body (different shmem)
    // Two-body writes sh_rep; triplet builder writes sh_tripj/k/ntrip only.
    // -----------------------------------------------------------------------
    if (tid == 0) {
        int ntrip = 0;
        for (int j = 0; j < natoms; ++j) {
            if (j == i) continue;
            const float rij = sh_D[i * max_atoms + j];
            if (rij >= acut) continue;
            for (int k = j + 1; k < natoms; ++k) {
                if (k == i) continue;
                const float rik = sh_D[i * max_atoms + k];
                if (rik >= acut) continue;
                // Safety guard (should not trigger for valid inputs)
                if (ntrip < max_pairs) {
                    sh_tripj[ntrip] = (short)j;
                    sh_tripk[ntrip] = (short)k;
                    ++ntrip;
                }
            }
        }
    ...
    // -----------------------------------------------------------------------
    // Two-body contributions — each thread handles one subset of neighbors
    // -----------------------------------------------------------------------
    ...
    for (int j = tid; j < natoms; j += BLOCK_SZ) {
        ...
        // Lognormal Gaussian construction (matches CPU generate_fchl_acsf)
        const float rij2    = rij * rij;
        const float t       = eta2 / fmaxf(rij2, EPS_F);
        const float log1pt  = log1pf(t);               // sigma^2 = log1pt
    ...
    __syncthreads();  // ensures both 2-body AND triplet list are ready

    // -----------------------------------------------------------------------
    // Three-body contributions — all threads stripe over triplets
    // -----------------------------------------------------------------------
    const int   ntrip       = *sh_ntrip;
    ...
    for (int t = tid; t < ntrip; t += BLOCK_SZ) {
        const int j = (int)sh_tripj[t];
        const int k = (int)sh_tripk[t];

        const float rij = sh_D[i * max_atoms + j];
        const float rik = sh_D[i * max_atoms + k];
        const float rjk = sh_D[j * max_atoms + k];

        // Cartesian coordinates
        const float Bx = sh_coords[i * 3 + 0];
        const float By = sh_coords[i * 3 + 1];
        const float Bz = sh_coords[i * 3 + 2];
        const float Ax = sh_coords[j * 3 + 0];
        const float Ay = sh_coords[j * 3 + 1];
        const float Az = sh_coords[j * 3 + 2];
        const float Cx = sh_coords[k * 3 + 0];
        const float Cy = sh_coords[k * 3 + 1];
        const float Cz = sh_coords[k * 3 + 2];

        const float inv_rij = 1.0f / fmaxf(rij, EPS_F);
        const float inv_rik = 1.0f / fmaxf(rik, EPS_F);
        const float inv_rjk = 1.0f / fmaxf(rjk, EPS_F);

        // Normalised edge vectors
        const float eij0 = (Ax - Bx) * inv_rij;
        const float eij1 = (Ay - By) * inv_rij;
        const float eij2 = (Az - Bz) * inv_rij;
        const float eik0 = (Cx - Bx) * inv_rik;
        const float eik1 = (Cy - By) * inv_rik;
        const float eik2 = (Cz - Bz) * inv_rik;
        const float ejk0 = (Cx - Ax) * inv_rjk;
        const float ejk1 = (Cy - Ay) * inv_rjk;
        const float ejk2 = (Cz - Az) * inv_rjk;

        // Cosines from dot products (no acos/cos calls)
        ...
        // ATM factor: (1 + 3*cos_i*cos_j*cos_k) * w / (rij*rik*rjk)^three_body_decay
        ...
        // Angular basis using Chebyshev recurrence on (cos_i, sin_i)
        //   o=1:  angular[0] = ang_w[0]*cos(θ),  angular[1] = ang_w[0]*sin(θ)
        //   o=3:  angular[2] = ang_w[1]*cos(3θ), angular[3] = ang_w[1]*sin(3θ)
        //   ...
        ...
        if (n_harm > 1) {
            ...
            float cn_2 = 1.0f, sn_2 = 0.0f;    // cos(0), sin(0)
            float cn_1 = cos_i, sn_1 = sin_i;   // cos(θ), sin(θ)
            int harm_stored = 1;
        ...
        // Accumulate: rep[base + l*nabasis + a] += angular[a] * radial_l * ksi3
...
__global__ static void fchl19_grad_kernel(
    ...
    for (int trip = tid; trip < ntrip; trip += BLOCK_SZ) {
        const int j = (int)sh_tripj[trip];
        const int k = (int)sh_tripk[trip];
...
torch::Tensor generate_fchl_acsf_cuda(
    ...
    // -----------------------------------------------------------------------
    const int max_pairs = max_atoms * (max_atoms - 1) / 2;
    // Account for 2-byte short arrays and 4-byte alignment before ntrip
    const int smem_bytes =
        max_atoms * 3  * (int)sizeof(float) +       // sh_coords
        max_atoms      * (int)sizeof(int)   +        // sh_Z
        max_atoms * max_atoms * (int)sizeof(float) + // sh_D
        rep_size       * (int)sizeof(float) +        // sh_rep
        max_pairs      * (int)sizeof(short) +        // sh_tripj
        max_pairs      * (int)sizeof(short) +        // sh_tripk
        2 +                                           // alignment padding
        (int)sizeof(int);                             // sh_ntrip

    // Sanity check
    {
        cudaDeviceProp prop{};
        CUDA_CHECK(cudaGetDeviceProperties(&prop, (int)coords.get_device()));
        TORCH_CHECK(
            smem_bytes <= (int)prop.sharedMemPerBlock,
            "Shared memory required (", smem_bytes, " bytes) exceeds device limit (",
            (int)prop.sharedMemPerBlock, " bytes). "
    ...
    fchl19_forward_kernel<<<grid, BLOCK_SZ, smem_bytes>>>(
...
std::tuple<torch::Tensor, torch::Tensor> generate_fchl_acsf_and_gradients_cuda(
    ...
    const int smem_bytes =
        ...
        max_atoms * max_atoms * (int)sizeof(float) +
        rep_size * (int)sizeof(float) +
        max_pairs * (int)sizeof(short) +
        max_pairs * (int)sizeof(short) +
        2 +
        (int)sizeof(int);
    ...
    TORCH_CHECK(
        smem_bytes <= (int)prop.sharedMemPerBlock,
        "Shared memory required (", smem_bytes, " bytes) exceeds device limit (",
        (int)prop.sharedMemPerBlock, " bytes). Reduce max_atoms or rep_size."
    );

    const int grid = nm * max_atoms;
    fchl19_grad_kernel<<<grid, BLOCK_SZ, smem_bytes>>>(
        coords.data_ptr<float>(),
        Q.data_ptr<int>(),
        N.data_ptr<int>(),
        rep.data_ptr<float>(),
