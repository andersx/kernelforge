...
#pragma omp for schedule(dynamic)
    ...
    #pragma omp atomic
                ...
                for (std::size_t r = 0; r < rep_size; ++r) {
                    std::memcpy(
                        dX_atom + r * ncols,
                        dX + i * dX_mol_stride + static_cast<std::size_t>(j) * dX_atom_stride +
                            r * (max_atoms * 3),
                        ncols * sizeof(double)
                    );
                }
...
void rff_gramian_elemental_rfp(
    ...
    if (chunk_size == 0) throw std::invalid_argument("rff_gramian_elemental_rfp: zero chunk_size");
