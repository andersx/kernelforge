...
py::array_t<double> inverse_distance(
    ...
    py::array_t<double> XT({3, n}, {buf.strides[1], buf.strides[0]}, static_cast<double *>(buf.ptr),
                           X);
...
py::array_t<double> kernel_symm_simple_py(
    py::array_t<double, py::array::forcecast | py::array::f_style> X, double alpha) {
    // Require (rep_size, n) in Fortran order; forcecast|f_style will copy if needed.
    auto xb = X.request();
    if (xb.ndim != 2) {
        throw std::runtime_error(
            "X must be 2D with shape (rep_size, n) in column-major (Fortran) order");
    }
    ...
    auto K =
        py::array_t<double>({n, n}, {sizeof(double), static_cast<ssize_t>(n) * sizeof(double)});

    kernel_symm_simple(static_cast<const double *>(xb.ptr), lda, n,
...
py::array_t<double> kernel_symm_blas_py(
    ...
    // Allocate K as Fortran-order (n x n): stride0 = 8, stride1 = n*8
    // auto K = py::array_t<double>({n, n}, {sizeof(double),
    // static_cast<ssize_t>(n)*sizeof(double)});
    auto ptr = aligned_alloc_64(static_cast<size_t>(n) * static_cast<size_t>(n));

    auto capsule = py::capsule(ptr, [](void *p) { aligned_free_64(p); });

    auto K = py::array_t<double>(
        {n, n}, {static_cast<ssize_t>(n) * sizeof(double), sizeof(double)},  // row-major
        ptr, capsule);

    kernel_symm_blas(static_cast<const double *>(xb.ptr), lda, n,
                     static_cast<double *>(K.request().ptr),
                     /*ldk=*/n, alpha);
...
PYBIND11_MODULE(_kernelforge, m) {
    ...
    m.def("inverse_distance", &inverse_distance,
          "Compute packed inverse distance matrix from (N,3) coordinates");
