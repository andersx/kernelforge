...
def run_md(
    model: Any,  # noqa: ANN401  # fitted KernelForge model with predict()
    ...
    ----------
    model:
        A fitted KernelForge model with force predictions (for example
        ``CudaLocalKRRModel`` or ``CudaLocalRFFModel``), wrapped by
