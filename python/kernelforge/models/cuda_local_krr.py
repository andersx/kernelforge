...
class CudaLocalKRRModel(BaseModel):
    ...
    **Only** ``energy_and_force`` training mode is supported.
    ...
    def _get_or_create_cached_graph(
        ...
        # Disabled for now: the current graph replay path can produce incorrect
        # first-call outputs for fixed-shape nm==1 inference, which is unsafe for MD.
        return None
        ...
        E_buf = torch.zeros((int(X_q.shape[0]),), device=X_q.device, dtype=torch.float32)
        F_buf = torch.zeros((naq_q,), device=X_q.device, dtype=torch.float32)
