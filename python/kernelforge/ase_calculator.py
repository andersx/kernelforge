"""ASE Calculator wrapping a fitted KernelForge model.
...
class _PredictModel(Protocol):
    """Minimal model interface required by KernelForgeCalculator."""
...
class KernelForgeCalculator(_AseCalculator):  # type: ignore[misc]
    """ASE Calculator backed by a fitted KernelForge model.

    Parameters
    ----------
    model:
        Any fitted KernelForge model exposing ``predict(coords_list, z_list,
        compute_energy=...)`` (for example ``CudaLocalKRRModel``,
        ``CudaLocalRFFModel``, or other CUDA/CPU models with force support).
        The model must have been trained so that force predictions are available
        (not ``energy_only``).
    units:
