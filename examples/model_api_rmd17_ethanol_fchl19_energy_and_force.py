"""
High-level model API — LocalKRRModel (FCHL19), energy+force training on MD17 Ethanol.

Trains an energy+force KRR model using the FCHL19 local kernel on the
MD17 ethanol dataset (555k DFT MD trajectories):
  1. Auto-download MD17 ethanol dataset (~first run only)
  2. Random train/test split
  3. Fit LocalKRRModel on energies + forces
...
Usage:
    uv run python examples/model_api_rmd17_ethanol_fchl19_energy_and_force.py
"""
...
SIGMA = 20.0
L2 = 1e-8
ELEMENTS = [1, 6, 8]  # H, C, O — all elements in ethanol

# ---------------------------------------------------------------------------
# Load data
# ---------------------------------------------------------------------------
print("=" * 60)
print("LocalKRRModel (FCHL19) — energy+force training on MD17 Ethanol")
print("=" * 60)
...
# Save and reload
# ---------------------------------------------------------------------------
save_path = "/tmp/rmd17_ethanol_fchl19_model.npz"
