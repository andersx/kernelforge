"""
Local RFF regression with force-only training — predict both energies and forces.

Trains on forces alone using elemental random Fourier features (RFF) with FCHL19
representations.  The gradient feature matrix G (D_rff, N_train*naq) plays the
role of the design matrix:

  G^T @ G @ w = G^T @ F_train_flat   (force-only normal equations)

Predictions:
  Forces  : G(x_test).T @ w   where G = rff_gradient_elemental (D_rff, N_test*naq)
  Energies: Z(x_test) @ w     where Z = rff_features_elemental  (N_test, D_rff)

RFF functions used
------------------
  Training (normal eqs, RFP): rff_gradient_gramian_elemental_rfp  →  (GtG_rfp, GtF)
...
def main():
    print("=" * 65)
    print("Local RFF: force-only training  →  predict energies + forces")
    print("=" * 65)
    ...
    # ------------------------------------------------------------------
    t0 = time.perf_counter()
    GtG_rfp, GtF = rff_gradient_gramian_elemental_rfp(X_tr, dX_tr, Q_tr, W, b, F_tr_flat)
    ...
    # ------------------------------------------------------------------
    G_tr = rff_gradient_elemental(X_tr, dX_tr, Q_tr, W, b)  # (D_rff, N_train*naq)
    F_tr_pred_flat = G_tr.T @ w  # (N_train*naq,)
