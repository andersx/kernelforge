"""
High-level model API — FCHL18KRRModel, energy+force training on small molecules.

Demonstrates energy_and_force training using the FCHL18 analytical kernel
with pre-split train/test sets:
  1. Load small_mols_mini_train.npz  (1000 molecules, H/C/N/O, up to 23 atoms)
  2. Fit FCHL18KRRModel on atomization energies + forces
...
Note: FCHL18 energy+force mode requires use_atm=False (ATM three-body
Hessian is not implemented) and a higher l2 (~1e-4) for Cholesky stability.

Usage:
    uv run python examples/model_api_small_mols_fchl18_energy_and_force.py
"""
...
N_TEST = 100
SIGMA = 2.5
L2 = 1e-6  # FCHL18 Hessian kernel is approx. PSD — needs higher l2
MAX_SIZE = 23  # must cover max atom count in dataset
...
# ---------------------------------------------------------------------------
print("=" * 60)
print("FCHL18KRRModel — energy+force training on small molecules")
print("=" * 60)
...
# ---------------------------------------------------------------------------
t0 = time.perf_counter()
model = FCHL18KRRModel(sigma=SIGMA, l2=L2, max_size=MAX_SIZE,  kernel_params={"use_atm": True},  )
...
# # Save and reload
# # ---------------------------------------------------------------------------
# save_path = "/tmp/small_mols_fchl18_model.npz"
