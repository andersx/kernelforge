...
def test_kernel_symm_shape_and_symmetry() -> None:
    ...
    K = _kernels.kernel_gaussian_symm(X, sigma=1.0)
...
def test_kernel_symm_against_numpy() -> None:
    ...
    K = _kernels.kernel_gaussian_symm(X, sigma)
    ...
    for i in range(n):
        for j in range(i + 1):
            ...
            K_ref[i, j] = np.exp(inv_2s2 * dist2)
...
def test_small_input() -> None:
    X = np.array([[0.0], [1.0]])
    sigma = 1.0 / np.sqrt(2.0)  # gives exp(-dist^2)
    K = _kernels.kernel_gaussian_symm(X, sigma)
...
def _ref_kernel_asymm(
    X1: NDArray[np.float64], X2: NDArray[np.float64], sigma: float
) -> NDArray[np.float64]:
    ...
    for i1 in range(n1):
        for i2 in range(n2):
            ...
            Kref[i1, i2] = np.exp(inv_2s2 * dist2)
    return Kref
...
def test_kernel_asymm_shape_and_values() -> None:
    ...
    K = _kernels.kernel_gaussian(X1, X2, sigma)
    assert K.shape == (n1, n2)

    Kref = _ref_kernel_asymm(X1, X2, sigma)
...
def test_kernel_asymm_small_case() -> None:
    ...
    K = _kernels.kernel_gaussian(X1, X2, sigma)
    # distances^2:
    # X1[0]=0 vs X2[0]=0 -> 0, exp(0)=1
    # X1[0]=0 vs X2[1]=2 -> 4, exp(-4)
    # X1[1]=1 vs X2[0]=0 -> 1, exp(-1)
    # X1[1]=1 vs X2[1]=2 -> 1, exp(-1)
